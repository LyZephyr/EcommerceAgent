...
**应对**：
- Prompt 中严格约束"仅基于以下商品信息回答，如果没有匹配商品请如实告知"
- 将商品卡片数据与 LLM 文本回复分离——卡片数据直接从检索结果取，不依赖 LLM 生成
- 可选：对 LLM 回复做后处理，校验提及的 product_id 是否存在于检索结果中

### 难点 2：Embedding 模型选型

**问题**：课题未提供 Embedding API Key，需自行解决。

**应对**：
- MVP 阶段使用 `text2vec-base-chinese`（sentence-transformers），本地 CPU 推理，100 条数据秒级完成
- 模型约 400MB，首次运行自动下载
- 如后续需要图文多模态 embedding，可切换为 CLIP 系列模型

### 难点 3：SSE 流式协议 + Android 解析

**问题**：需要在 SSE 流中同时传输逐字文本和结构化商品数据，客户端要正确解析。

**应对**：
- 设计多事件类型 SSE 协议（`token` / `product` / `done`）
- Android 端使用 OkHttp 的 `EventSource` 接口，按 event 类型分发处理
- 商品数据在检索完成后一次性发送，不依赖 LLM 输出解析
